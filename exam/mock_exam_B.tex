...
Consider the \emph{path graph} with 5 nodes $a$--$b$--$c$--$d$--$e$.

The \emph{clustering coefficient} of a node $u$ is defined as
\[
  c_u = \frac{|\{(v_1,v_2)\in E : v_1,v_2\in\mathcal{N}(u)\}|}{\binom{d_u}{2}}, \tag{9}
\]
where $d_u$ is the degree of $u$ (use $c_u=0$ by convention when $d_u<2$).

\textbf{Question G.1:} What are the clustering coefficients of node $b$ and node $a$?
...
where $\lambda$ is the largest eigenvalue of the adjacency matrix.  For the path graph above,
$e_a=e_e=0.29$ and $e_b=e_d=0.50$.

\textbf{Question G.2:} What is the eigenvector centrality $e_c$?
